#!/bin/bash
# run_roberta_cur.sh – CurriculumDocRE training with RoBERTa-large

# GPU selection (default 0)
export CUDA_VISIBLE_DEVICES=${1:-0}

# Paths (relative to project root, as we run from there)
DATA_DIR="data/redocred"
TRAIN_FILE="train.json"
DEV_FILE="dev.json"
TEST_FILE="test.json"
SAVE_PATH="output/roberta_curriculum"
...
# Training hyperparameters (RoBERTa-large)
BATCH_SIZE=4
GRAD_ACC=2
LR=3e-5
WARMUP_RATIO=0.06
...
# Augmentation (0 = off, 1 = on)
AUGMENT=0
AUGMENT_FACTOR=1

# WandB logging (0 = off, 1 = on)
USE_WANDB=0
WANDB_PROJECT="CurriculumDocRE"
WANDB_NAME="roberta_curriculum"
